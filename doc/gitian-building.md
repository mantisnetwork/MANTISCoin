...
*Setup instructions for a gitian build of MANTISCoin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the MANTISCoin
...
to the MANTISCoin GitHub release page.
...
- [Building MANTISCoin](#building-MANTIScoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/MANTIScoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/MANTIScoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/MANTIScoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/MANTIScoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/MANTIScoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/MANTIScoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/MANTIScoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@MANTIScoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/MANTIScoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/MANTIScoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/MANTIScoin-osx-*-build.assert
```
