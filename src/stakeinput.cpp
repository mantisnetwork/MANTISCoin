...
bool CMntisStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CMntisStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CMntisStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CMntisStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CMntisStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CMntisStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CMntisStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CMntisStake::GetUniqueness() const
{
    //The unique identifier for a MNTIS stake is the outpoint
...
CBlockIndex* CMntisStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CMntisStake::ContextCheck(int nHeight, uint32_t nTime)
{
