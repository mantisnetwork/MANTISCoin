...
#if defined(NDEBUG)
#error "MANTISCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umntis) are considered zero fee (for relaying, mining and transaction creation)
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 290000 && nHeight > 270000) {
		nSubsidy = 2.25 * COIN;
	}
	else if (nHeight <= 72418173 && nHeight > 290000) {
		nSubsidy = 2.30 * COIN;
	}
	else if (nHeight == 72814174) {
		nSubsidy = 2.10 * COIN;
	}
	else {
		nSubsidy = 0;
...
{
    util::ThreadRename("MANTIScoin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> mntisInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                mntisInputs.push_back(stakeIn);
        }
        const bool hasMNTISInputs = !mntisInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasMNTISInputs) {
                    ...
                    for (const CTxIn& mntisIn : mntisInputs)
                        if(mntisIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasMNTISInputs) {
                            for (const CTxIn& stakeIn : mntisInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // MANTISCoin: We use certain sporks during IBD, so check to see if they are
