...
#include "qt/MANTIScoin/forms/ui_addresseswidget.h"
#include "qt/MANTIScoin/addresslabelrow.h"
#include "qt/MANTIScoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(MANTISCoinGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination mntisAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(mntisAdd, label.toUtf8().constData(),
