...
#include "qt/MANTIScoin/forms/ui_snackbar.h"
#include "qt/MANTIScoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(MANTISCoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &MANTISCoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
