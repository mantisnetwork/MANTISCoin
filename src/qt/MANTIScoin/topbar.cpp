...
#include "qt/MANTIScoin/topbar.h"
#include "qt/MANTIScoin/forms/ui_topbar.h"
#include "qt/MANTIScoin/lockunlock.h"
#include "qt/MANTIScoin/qtutils.h"
#include "qt/MANTIScoin/receivedialog.h"
#include "qt/MANTIScoin/loadingdialog.h"
...
TopBar::TopBar(MANTISCoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopMntis}, "amount-small-topbar");
    setCssProperty({ui->labelAmountMntis}, "amount-topbar");
    setCssProperty({ui->labelPendingMntis, ui->labelImmatureMntis}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("MANTIS Coin")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // MNTIS Total
    QString totalMntis = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);

    // MNTIS
    // Top
    ui->labelAmountTopMntis->setText(totalMntis);
    // Expanded
    ui->labelAmountMntis->setText(totalMntis);
    ui->labelPendingMntis->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureMntis->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
