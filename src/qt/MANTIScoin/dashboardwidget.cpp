...
#include "qt/MANTIScoin/dashboardwidget.h"
#include "qt/MANTIScoin/forms/ui_dashboardwidget.h"
#include "qt/MANTIScoin/sendconfirmdialog.h"
#include "qt/MANTIScoin/txrow.h"
...
DashboardWidget::DashboardWidget(MANTISCoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	setCssProperty(ui->labelSquareMntis, "square-chart-mntis");
	setCssProperty(ui->labelMntis, "text-chart-mntis");
	...
	setCssProperty(ui->labelAmountMntis, "text-stake-mntis-disable");
	...
	if (window)
		connect(window, &MANTISCoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
	if (walletModel && walletModel->getOptionsModel()) {
		...
		connect(ui->pushImgEmpty, &QPushButton::clicked, window, &MANTISCoinGUI::openFAQ);
		connect(ui->btnHowTo, &QPushButton::clicked, window, &MANTISCoinGUI::openFAQ);
	...
	// update the display unit, to not use the default ("MNTIS")
	updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
	...
	chartData->amountsByCache = getAmountBy(); // pair MNTIS
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData->amountsByCache.contains(num)) {
			...
			mntis = (pair.first != 0) ? pair.first / 100000000 : 0;
			chartData->totalMntis += pair.first;
		}

		chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesMntis.append(mntis);

		int max = std::max(mntis, mntis);
...
void DashboardWidget::onChartRefreshed()
{
	...
	set0->append(chartData->valuesMntis);

	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalMntis > 0) {
		setCssProperty(ui->labelAmountMntis, "text-stake-mntis");
	}
	else {
		setCssProperty(ui->labelAmountMntis, "text-stake-mntis-disable");
	}
	forceUpdateStyle({ ui->labelAmountMntis });
	ui->labelAmountMntis->setText(GUIUtil::formatBalance(chartData->totalMntis, nDisplayUnit));
