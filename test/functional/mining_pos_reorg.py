...
from test_framework.test_framework import MANTISCoinTestFramework
...
class ReorgStakeTest(MANTISCoinTestFramework):
    ...
    def check_money_supply(self, expected_bc, expected_zmntis):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected MNTIS and zMNTIS supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_bc))
            for denom in node['zMNTISsupply']:
                assert_equal(node['zMNTISsupply'][denom], DecimalAmt(expected_zmntis[denom]))
    ...
    def run_test(self):
        ...
        # Check MNTIS and zMNTIS supply at the beginning
        # ------------------------------------------
        # zMNTIS supply: 2 coins for each denomination
        expected_zmntis_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # MNTIS supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zmntis_supply)
        ...
        # Verify that MNTIS and zMNTIS supplies were properly updated after the spends and reorgs
        self.log.info("Check MNTIS and zMNTIS supply...")
        ...
        expected_zmntis_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zmntis_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zmntis_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zmntis_supply)
