...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/MANTISCoinParams"
    else
        PARAMS_DIR="$HOME/.MANTIScoin-params"
    fi
fi
...
Failed to fetch the MANTISCoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
MANTISCoin - fetch-params.sh

This script will fetch the MANTISCoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common MANTISCoin zkSNARK parameters. Note that it is
